...
describe('drawNeonOverlay', () => {
  it('draws zone washes and dividers without metronome beat dots', () => {
    const ctx = {
      save: vi.fn(),
      restore: vi.fn(),
      beginPath: vi.fn(),
    ...
    // No beat dots
    expect((ctx.arc as unknown as ReturnType<typeof vi.fn>).mock.calls.length).toBe(0);
...
describe('OverlayCanvas', () => {
  ...
  it('does not call pose draw callback when disabled', () => {
    ...
    render(<OverlayCanvas video={video} enabled={false} onDraw={drawSpy} />);
